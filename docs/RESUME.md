...
## Current state
- **main** = Sprints 0 + 1 + 2 + 2.5 merged. **Sprint 3 is decomposed into 6 slices; slice 1 (assessment config + subject assignment)** built + browser-QA'd on `sprint-3-assessment-config` (ready to merge).
...
- **Remaining Sprint 3 slices (each its own spec→plan→build):** 2 score entry + auto-calc · 3 review + anomaly (class/subject master sheets, z-score) · 4 release workflow + immutability (frozen ResultSheet, position) · 5 report-card PDF + public verification · 6 reveal animation (Framer Motion).
...
- **apps/web has vitest** (added Sprint 2.5): **20 web tests** (18 offline + 2 grade). API e2e now includes the 14-test assessment suite. All builds + typecheck + lint green.
...
- Assessment config (Sprint 3 slice 1): `apps/api/src/modules/assessment/` (types/boundaries/subject-assignments) + Settings → Assessment web UI. The config spine for the rest of Sprint 3.
- Web: `(app)` shell + sidebar (Dashboard, Students, Staff, Classes, Attendance, Settings), login, onboarding wizard, Settings → Assessment.

## Next steps (in order)
1. **Merge `sprint-3-assessment-config` → main** once satisfied (`git checkout main && git merge sprint-3-assessment-config --no-edit`), then branch **Sprint 3 slice 2 (score entry + auto-calc)** — it consumes this config and must guard structure edits once scores exist.
2. **Sprint 3 slices 3–6** (review/anomaly → release/immutability → report-card PDF + verify → reveal animation), each its own spec→plan→build.
