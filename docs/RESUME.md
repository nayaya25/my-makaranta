...
## Current state
- **main** = Sprints 0–2.5 + Sprint 3 slice 1 merged. **Sprint 3 slice 2 (score entry + auto-calc)** built + browser-QA'd on `sprint-3-score-entry` (ready to merge).
...
- **Remaining Sprint 3 slices (each its own spec→plan→build):** 3 review + anomaly (class/subject master sheets, z-score; consumes `Score` + `computeSubjectResult`) · 4 release workflow + immutability (frozen ResultSheet, position) · 5 report-card PDF + public verification · 6 reveal animation (Framer Motion).
...
- **apps/web has vitest** (added Sprint 2.5): **22 web tests** (18 offline + 2 grade + 2 gradebook). API e2e: 19-test assessment suite (config + scores + guard + cross-tenant). All builds + typecheck + lint green.
...
- Assessment (Sprint 3 s1+s2): `apps/api/src/modules/assessment/` — config (types/boundaries/subject-assignments) + `Score` model + gradebook (GET + batch save) + `computeSubjectResult` + structure guard. Web: Settings → Assessment config UI + `/gradebook` score-entry grid.
- Web: `(app)` shell + sidebar (Dashboard, Students, Staff, Classes, Attendance, Gradebook, Settings), login, onboarding wizard, Settings → Assessment.

## Next steps (in order)
1. **Merge `sprint-3-score-entry` → main** once satisfied (`git checkout main && git merge sprint-3-score-entry --no-edit`), then branch **Sprint 3 slice 3 (review + anomaly)** — class-master / subject-master sheets + z-score outliers; consumes `Score` + `computeSubjectResult`.
2. **Sprint 3 slices 4–6** (release/immutability → report-card PDF + verify → reveal animation), each its own spec→plan→build.
